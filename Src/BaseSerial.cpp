...
BaseSerial::Descriptor::Descriptor(Descriptor &&rhs) : Fd{rhs.Fd} {
  rhs.Fd = -1;
}
...
std::runtime_error MakeTTYExceptionErrno(const BaseSerial::TTYConfig &conf,
                                         const std::string &msg) {
  return MakeTTYException(
      conf, msg + ": errno #" + std::to_string(errno) + ": " + strerror(errno));
}
...
void BaseSerial::Configure(const TTYConfig &ttyConfig) {
  ...
  if (!ttyConfig.ParityBit) tty.c_cflag &= ~PARENB;
  if (ttyConfig.ParityBit) tty.c_cflag |= PARENB;
...
void BaseSerial::Send(const uint8_t *buf, size_t size) const {
  if (!buf) throw MakeTTYException(TTY, "Send(): buf is NULL");
...
void BaseSerial::Recv(uint8_t *buf, size_t size) const {
  if (!buf) throw MakeTTYException(TTY, "Recv(): buf is NULL");
